...
Corrupt JSON, invalid companies and unavailable browser storage fall back to an in-memory store. Legacy `interview-mastered-*` and `interview-active-company` keys are read once and migrated. Older digest-like IDs are translated by `js/legacy-id-map.js` to explicit immutable IDs before state normalization.
