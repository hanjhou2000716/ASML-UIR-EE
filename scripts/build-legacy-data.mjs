...
const renderer = `(function () {
  ...
  const cardHtml = question => '<article class="qa-card bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden" data-question-id="' + question.id + '"><button data-action="toggle-card" class="btn-press w-full p-4 text-left flex justify-between gap-3 items-center" aria-expanded="false"><span class="font-bold text-[14px] text-slate-700"><span class="tag tag-' + question.priority + '">' + question.priority.toUpperCase() + '</span><br>' + escapeHtml(question.question) + '</span><span data-icon="chevron" class=""></span></button><div class="accordion-wrapper"><div class="accordion-inner"><div class="px-4 pb-4 text-[13px] text-slate-600 border-t pt-3 leading-relaxed">' + question.answerHtml + '</div></div></div></article>';
  const categoryHtml = (companyId, category, index) => '<div id="content-' + companyId + '-' + category.id + '" class="sub-content ' + (index === 0 ? 'block' : 'hidden') + ' space-y-3">' + category.introHtml + '<div class="study-toolbar flex items-center gap-2"><div class="search-box-wrapper"><span data-icon="search" class=""></span><input type="search" class="study-search" aria-label="搜尋本分組題庫" placeholder="搜尋本分組題目、關鍵字或案例..." data-action="filter" data-content="content-' + companyId + '-' + category.id + '"></div><select class="status-filter" aria-label="篩選練習狀態" data-action="status-filter" data-content="content-' + companyId + '-' + category.id + '"><option value="all">全部</option><option value="unmastered">未掌握</option><option value="practiced">練習過</option><option value="mastered">已掌握</option></select><span class="question-count" data-result-count>' + category.questions.length + ' 題</span><button data-action="random" data-content="content-' + companyId + '-' + category.id + '" aria-label="隨機抽一題練習" class="btn-press flex-shrink-0 bg-[#d9823b] text-white px-3 py-2 rounded-lg text-xs font-black"><span data-icon="dice" class=""></span><span class="hidden sm:inline ml-1">抽題</span></button></div>' + category.questions.map(cardHtml).join('') + '</div>';
